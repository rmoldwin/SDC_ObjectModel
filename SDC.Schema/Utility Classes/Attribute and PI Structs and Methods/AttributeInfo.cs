...
namespace SDC.Schema
{
    ...
    public readonly record struct DifNodeIET2
    {
            ...
            )
        {
            ...
            DifNodeIET2 test = new(string.Empty, true, true, true, true, true, new());
        }
        /// <summary>
    ...
    /// <summary>
    /// Represents metadata for a single XML attribute on an SDC representation of an xml element node.<br/>
    /// Describes the attribute changes that took place on that SDC node, as it changes from an old SDC template to a newer version of that template.<br/>
    ...
    public readonly record struct AttInfoDif
    {
            ...
            , string elementName, string propertyName, string displayName)
        {
            ...
            // Determine if attribute was added in New version
            // addedInNew is true when aiNew has a value but aiPrev is null or default
            // null indicates the comparison doesn't apply (e.g., both null or both present)
            if (aiNew.HasValue && aiNew.Value.Name != null)
            {
                // aiNew exists - check if aiPrev exists
                if (!aiPrev.HasValue || aiPrev.Value.Name == null)
                {
                    this.addedInNew = true;
                }
                else
                {
                    // Both exist - not added
                    this.addedInNew = false;
                }
            }
            else
            {
                // aiNew doesn't exist - not added
                this.addedInNew = null;
            }

            // Determine if attribute was removed in New version
            // removedInNew is true when aiPrev has a value but aiNew is null or default
            // null indicates the comparison doesn't apply
            if (aiPrev.HasValue && aiPrev.Value.Name != null)
            {
                // aiPrev exists - check if aiNew exists
                if (!aiNew.HasValue || aiNew.Value.Name == null)
                {
                    this.removedInNew = true;
                }
                else
                {
                    // Both exist - not removed
                    this.removedInNew = false;
                }
            }
            else
            {
                // aiPrev doesn't exist - not removed
                this.removedInNew = null;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// True if this attribute was added in the New node
        /// </summary>
        public readonly bool? addedInNew;
        /// <summary>
        /// True if this attribute was present in the Prev node, but absent in the New node
        /// Use aiPrev.Value.value to get the value of the removed attribute in the previous version, if needed.
        /// </summary>
        public readonly bool? removedInNew;
    ...
    public readonly record struct DifNodeIET( //using simplified init-only, readonly, auto-implemented property syntax
            ...
            //TODO: Add fields for added and removed attributes
            //uses attribute name for key
            ...
            //Dictionary<string, AttributeInfo>? unChangedAttributes,

            List<BaseType>? addedSubNodes,
            List<BaseType>? removedSubNodes,
            //TODO: add field for changed sub-nodes (with changed attributes?)
            //TODO: add field for unchanged sub-nodes (with changed attributes?)
            //TODO: add field for all sub-nodes
            //TODO: add IsChanged (including attribute changes) on only this node
            //TODO: add extension methods on BaseType to get added/removed/changed/unchanged sub-nodes
