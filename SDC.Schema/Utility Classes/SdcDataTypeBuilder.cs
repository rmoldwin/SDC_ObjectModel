...
namespace SDC.Schema
{
    ...
    internal static class SdcDataTypeBuilder
    {
            ...
            try
            {
                ...
                switch (dataTypeEnum)
                {
                    ...
                    case ItemChoiceType.anyURI:
                        // XSD anyURI is an IRI-reference (RFC 3986/3987): absolute URIs, relative paths,
                        // fragment-only refs ("#top"), bare strings, Unicode IRI chars, and the empty string
                        // (same-document reference, RFC 3986 §4.4) are all valid. A scheme is NOT required.
                        // Validation mirrors .NET's internal XmlConvert.TryToUri (4 steps): trim whitespace,
                        // reject all-whitespace strings, reject "##" (two fragment delimiters), Uri.TryCreate.
                        {
                            ...
                            if (value != null)
                            {
                                ...
                                {
                                    if (IsValidAnyUri(s)) tmp = s;
                                    else RecordAndRaise(
                                        $"Supplied value '{s}' is not a valid xs:anyURI. " +
                                        "xs:anyURI accepts any IRI-reference: absolute URIs (e.g. \"https://example.com/path\"), " +
                                        "relative paths (\"../foo\"), fragment references (\"#section\"), bare strings, " +
                                        "and the empty string are all valid. " +
                                        "Strings containing \"##\", and all-whitespace strings are not accepted.",
                                        dt, value, rfParent);
                                }
                                else RecordAndRaise(
                                    $"Supplied value parameter (type {value.GetType().Name}) could not be used as xs:anyURI; a string is required.",
                                    dt, value, rfParent);
                            }
        ...
        /// <summary>
        /// Validates a candidate xs:anyURI value using the same four-step logic as .NET's internal
        /// <c>XmlConvert.TryToUri</c>, which is what the .NET XSD schema validator calls for xs:anyURI.
        /// </summary>
        /// <remarks>
        /// Steps: (1) trim XML whitespace (XSD whiteSpace=collapse); (2) reject all-whitespace strings
        /// (empty after trim, non-empty before); (3) reject strings containing <c>##</c> (only one
        /// fragment delimiter per RFC 3986); (4) <see cref="Uri.TryCreate"/> structural gate.
        /// A genuinely empty string <c>""</c> is a valid same-document URI-reference (RFC 3986 §4.4)
        /// and is accepted immediately before trimming.
        /// </remarks>
