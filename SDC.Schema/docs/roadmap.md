...
| Status | Item | Issue |
|---|---|---|
| Planned | Migrate the two public GitHub gists ("SDC OM Validation" and "SDC OM Rules") into `docs/architecture/validation.md` and `docs/architecture/rules.md`, then delete the gists | TBD |
| Planned | Consolidate `SDC.Schema.Tests/Documentation` technical docs (XML Schema Definition, or XSD, vs. .NET type divergence notes; OM tree-stability notes; thread-safety investigation notes) into `docs/architecture/`, archiving originals | TBD |
| Planned | Move session handoff/kickstart documents out of `SDC.Schema.Tests/Documentation` into a new top-level `sessions/` folder | TBD |
