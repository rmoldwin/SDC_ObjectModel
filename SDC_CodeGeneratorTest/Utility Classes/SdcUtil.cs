    ...
    public static class SdcUtil
    {
            ...
            bool getNames = true)
        {
            ...
            foreach (var propInfo in ieProps) //loop through IEnumerable PropertyInfo objects in par
            {
                                    ...
                                    //Look for XmlElementAttributes on the IEnumerable property object in par 
                {
                    ...
                    if (xmlAtts.Length > 0) //we found the correct PropertyInfo object, which has XmlElementAttributes
                    {
            ...
            string ReflectElementName(PropertyInfo pi, out int xmlOrder, int itemIndex = -1)
            {
                ...
                if (dtAtts != null && dtAtts.Length > 0) xmlOrder = dtAtts.ToArray()[0].Order;
                ...
                if (getNames)
                {
                    ...
                    if (dtAtts.Length == 1) return dtAtts.ToArray()[0].ElementName;
                ...
                {
                    if (choiceIdentifierObject is Enum e)
                        return e.ToString();

                    if (choiceIdentifierObject is IEnumerable ie && arrayIndex > -1)
                    {
        ...
        public static string ReflectSdcElement(PropertyInfo piItem, BaseType item, out int xmlOrder, int itemIndex = -1)
        {
            ...
            object ItemChoiceEnum(PropertyInfo pi)
            {
                ...
                if (enumObj is Enum e) return e;
                if (enumObj is IEnumerable<Enum>[] ie) return ie;
                return null!;
                //!+---------Local Function------------------------------
                string ItemChoiceEnumName(PropertyInfo pi)
                {
