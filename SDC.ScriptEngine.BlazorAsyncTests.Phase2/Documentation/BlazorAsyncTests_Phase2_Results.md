...
| Cat 4 / `CompareTrees_ConcurrentInstances_4Threads_SameResults` and `CompareTrees_LockContention_NoDeadlock` | ✅ Passed after static traversal lock | The `Arg_LongerThanDestArray` race reproduced before the fix, then passed after `CompareTrees<T>` changed from per-instance traversal locks to a static traversal lock. Latest browser run: V1->V5 serial/timing 3699 ms, 4-thread concurrent V1->V5 3930 ms, V1->V2 lock contention 122 ms. | Keep the `CompareTrees<T>.TraversalLock` fix; continue using Cat4 as the correctness gate for CompareTrees concurrency. |
...
| Cat 6 / `CompareTrees Performance Measurements` | ✅ Quick throughput: 2.65x; heavy reference: 917 ms | Separates performance measurement from correctness/watchdog tests. Latest embedded-browser quick run measured sequential 4x V1->V2 at 78 ms and parallel 4x at 29 ms, with IETattDiffs=21; heavy V1->V5 reference measured 917 ms with IETattDiffs=907. | Keep as measurement-only evidence that 4-thread WASM can improve independent CompareTrees throughput; Cat4 remains the separate correctness gate. |
